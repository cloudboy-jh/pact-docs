...
```bash
# Import your existing setup into pact
pact read

# Or sync from an existing pact config
pact sync
```

Use `pact read` to bootstrap from your current environment, or `pact sync` to apply an existing configuration.
